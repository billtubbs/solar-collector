...
\title{Parabolic Trough Solar Collector Simulation Model}
...
The density of the fluid, $\rho_f$, as a function of its temperature, $T_f$, in \si{\kelvin} was simulated using the quadratic model:
\begin{equation}
...
The specific heat capacity of the fluid, $c_{p,f}$, as a function of its temperature was simulated using the linear model:
\begin{equation}
...
\caption{Heat capacity model.}
\label{fig:heat_capacity}
\end{figure}

The thermal conductivity of the fluid, $k_f$, as a function of its temperature was simulated using the linear model:
\begin{equation}
...
\caption{Thermal conductivity model.}
\label{fig:thermal_conductivity}
\end{figure}

The dynamic viscosity of the fluid, $\mu_f$, as a function of its temperature was simulated using the Andrade equation with an offset parameter:
\begin{equation}
...
The vapor pressure of the fluid, $P_f$, as a function of its temperature was simulated using the Antoine equation, where $T_C = T_f - 273.15$ is the temperature in \si{\degreeCelsius}:
\begin{equation}
...
\caption{Vapor pressure model.}
\label{fig:vapor_pressure}
\end{figure}

\section{Solar Collector Model} \label{sec:solar_collector}
...
$D_{ax}$ & Axial dispersion coefficient & \si{\square\meter\per\second} \\
...
The model uses mass flow rate $\dot{m}(t)$ as the primary flow input since it is conserved along the pipe length. The local fluid velocity is computed from mass conservation:
\begin{equation}
...
\begin{equation}
\rho_f c_{p,f} \frac{\partial T_f}{\partial t} + \rho_f c_{p,f} v \frac{\partial T_f}{\partial x} = \rho_f c_{p,f} D_{ax} \frac{\partial^2 T_f}{\partial x^2} + \dot{q}_{\text{wall}\rightarrow\text{fluid}}
\label{eq:fluid_pde}
\end{equation}
...
Note that in \eqref{eq:fluid_pde}, the fluid properties $\rho_f$, $c_{p,f}$, and the heat transfer coefficient $h_{\text{int}}$ may be either constant or functions of the local fluid temperature $T_f(t,x)$. The temperature-dependent correlations for fluid properties are given in Section~\ref{sec:fluid_properties}.

The axial dispersion coefficient $D_{ax}$ characterizes the effective spreading of heat in the axial direction due to turbulent mixing. For turbulent pipe flow, this coefficient is typically 2--3 orders of magnitude larger than the molecular thermal diffusivity $\alpha = k_f/(\rho_f c_{p,f})$, reflecting the dominant role of turbulent eddies in axial heat transport. The model treats $D_{ax}$ as a constant parameter.
...
The internal heat transfer coefficient $h_{\text{int}}$ characterizes the convective heat transfer between the pipe wall and the flowing fluid. It is calculated using the Dittus-Boelter correlation \citep{incropera2007heat}, an empirical relationship for turbulent flow in smooth pipes. This correlation relates the Nusselt number to the Reynolds and Prandtl numbers and is valid for fully developed turbulent flow with moderate temperature differences.

The Dittus-Boelter correlation is given by:
\begin{equation}
\text{Nu} = 0.023 \cdot \text{Re}^{0.8} \cdot \text{Pr}^{0.4} \label{eq:dittus-boelter}
\end{equation}
where $\text{Nu} = h D / k_f$ is the Nusselt number, $\text{Re} = \rho_f v D / \mu_f$ is the Reynolds number, and $\text{Pr} = \mu_f c_{p,f} / k_f$ is the Prandtl number. The heat transfer coefficient is then obtained from the Nusselt number:
\begin{equation}
h_{\text{int}} = \frac{\text{Nu} \cdot k_f}{D} \label{eq:htc}
\end{equation}
This correlation is valid for $\text{Re} > 10{,}000$ and $0.7 \leq \text{Pr} \leq 160$, with an accuracy of approximately $\pm 25\%$ for most engineering applications.
...
Figures~\ref{fig:re_vs_v} and \ref{fig:h_vs_v_and_Tf} show the Reynolds number and heat transfer coefficient as functions of fluid velocity for the absorber pipe.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{images/reynolds_number_vs_velocity.png}
\caption{Reynolds number for absorber pipe flow.}
\label{fig:re_vs_v}
\end{figure}
...
\subsection{Numerical Solution} \label{sec:numerical_solution}

The coupled PDEs are solved using the method of lines with finite difference discretization. The spatial domain is discretized using central differences (second-order accuracy), while the temporal domain uses backward Euler (implicit) discretization for numerical stability. The discretization grid can be specified with two parameters, $n_x$ and $n_t$ which define the number of points in the spatial and time dimensions, respectively.  The desired duration of the simulation is specified by setting $t_{final}$.

The resulting system of differential-algebraic equations (DAEs) is solved using the IPOPT nonlinear optimization solver within the Pyomo framework \citep{bynum2021pyomo, hart2011pyomo}.
...
\hline
Density & $\rho_f$ & 671.0 & \si{\kilogram\per\cubic\meter} \\
Specific heat & $c_{p,f}$ & 2086 & \si{\joule\per\kilogram\per\kelvin} \\
Thermal conductivity & $k_f$ & 0.0824 & \si{\watt\per\meter\per\kelvin} \\
Dynamic viscosity & $\mu_f$ & $4.7 \times 10^{-4}$ & \si{\pascal\second} \\
Axial dispersion coeff. & $D_{ax}$ & $1 \times 10^{-4}$ & \si{\square\meter\per\second} \\
\hline
\multicolumn{4}{l}{\textit{Absorber pipe (steel)}} \\
\hline
Length & $L$ & 96 & \si{\meter} \\
